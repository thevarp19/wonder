...
import { AdminCancelledTable } from "@/modules/order/components/AdminOrders/AdminCancelledTable";
import { deliveryModes, items } from "@/modules/order/const";
import { useGetCancelledOrderAdmin } from "@/modules/order/queries";
...
interface AdminOrderCancelledPageProps {}

export const AdminOrderCancelledPage: FC<AdminOrderCancelledPageProps> = () => {
    const [searchValue, setSearchValue] = useState("");
    const debouncedSearchValue = useDebounce(searchValue, 500);
    const [current, setCurrent] = useState("all");
    const [deliveryMode, setDeliveryMode] = useState<DeliveryMode>("ALL");
    const [page, setPage] = useState(0);
    const { data: orders, isPending } = useGetCancelledOrderAdmin(
        page,
        10,
