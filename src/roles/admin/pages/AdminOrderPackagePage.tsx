...
import { AdminPackageTable } from "@/modules/order/components/AdminOrders/AdminPackageTable";
import { deliveryModes, items } from "@/modules/order/const";
import { useGetPackageOrderAdmin } from "@/modules/order/queries";
...
interface AdminOrderPackagePageProps {}

export const AdminOrderPackagePage: FC<AdminOrderPackagePageProps> = () => {
    const [searchValue, setSearchValue] = useState("");
    const debouncedSearchValue = useDebounce(searchValue, 500);
    const [current, setCurrent] = useState("all");
    const [deliveryMode, setDeliveryMode] = useState<DeliveryMode>("ALL");
    const [page, setPage] = useState(0);
    const { data: orders, isPending } = useGetPackageOrderAdmin(
        page,
        10,
